...
export default function TradeBadge({ trade, name = "TradeBadge", ...rest }: PropsWithChildren<ComponentProps>) {
  const isActive = trade?.profit == null

  const value = isActive ? trade?.margin?.currentProfit : trade?.profit
